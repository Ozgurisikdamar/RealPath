# SKILLS.md — relpath.dev Playbook'ları ve Claude Code Skill'leri
...
>
> relpath.dev — açık kaynak, self-hostable, **local-first** relational prediction engine. Bir veritabanı bağla, tahmine dayalı bir soruyu düz dille veya PQL ile sor, **açıklamalı** (explained) bir cevap al — veriyi hiçbir yere taşımadan. Kumo.AI / KumoRFM'in açık kaynak karşılığı.
...
5. **NL→PQL için Claude yolu `ANTHROPIC_API_KEY` ister**; yoksa offline template fallback kullanılır. Varsayılan model `claude-sonnet-4-6` (env `RELPATH_LLM_MODEL` ile override).
6. **Local-first ürün tezidir**: veri makineden çıkmak zorunda olmamalı. DuckDB varsayılan kalır.
7. **Windows'ta `PYTHONUTF8=1`** tercih et (veya `relpath._io.sprint`'e güven) ki Türkçe metin cp1252 console'da çökmesin.
...
| Editable install | `.venv\Scripts\python.exe -m pip install -e .` (entry point script: `relpath`) |
...
1. `relpath/templates.py` içinde `TEMPLATES` registry'sine yeni bir giriş ekle (`desc` + `params`). Mevcutlar: `churn`, `forecast`, `fraud`.
...
3. `relpath/engine.py` içinde `Engine`'e bir metot ekle (mevcutlar: `churn()`, `forecast()`, `fraud()`). Metot builder'ı çağırıp `self.predict(...)` ile çalıştırmalı.
...
**Dosyalar:** `relpath/templates.py`, `relpath/engine.py`, `tests/test_pql_parser.py` (veya yeni test dosyası), `tests/conftest.py` (`sample_db` fixture).
...
# ve uçtan uca:
.venv\Scripts\python.exe -m relpath.cli predict "<builder'ın ürettiği PQL>" --db data/shop.duckdb --explain
```
...
1. `relpath/connect.py` içinde `DuckDBBackend` arayüzünü birebir uygulayan yeni bir backend sınıfı yaz. Sağlanması gereken yüzey:
...
**Dosyalar:** `relpath/connect.py` (ana iş), `relpath/schema.py` (dtype normalize uyumu), `tests/` (yeni connector için bir smoke test).
...
1. `relpath/pql/ast.py` → `AGGS` setine yeni fonksiyonu ekle (gerekirse alias'ı parser'da çöz, `MEAN→AVG` gibi).
2. `relpath/pql/parser.py` → `_parse_target()` mantığını gözden geçir. Regex (`_AGG_RE`) zaten `[A-Za-z_]+` func adını yakalar; `func not in AGGS` kontrolü yeni aggi otomatik kabul eder. `func != "COUNT" and col is None` kuralına dikkat (yeni agg `*` ile çalışmıyorsa column zorunluluğunu koru).
3. `relpath/pql/compile.py` → SQL mapping'i ekle. Label/event CTE'lerinde bu aggregate'i üreten SQL'i ve regression/classification eşlemesini güncelle (`build_split` events CTE + COALESCE).
4. `tests/test_pql_parser.py` → yeni agg için en az bir geçerli ve gerekirse bir geçersiz (`PQLSyntaxError`) case ekle.

**Dosyalar:** `relpath/pql/ast.py`, `relpath/pql/parser.py`, `relpath/pql/compile.py`, `tests/test_pql_parser.py`.

**Doğrulama:**

```powershell
.venv\Scripts\python.exe -m pytest tests/test_pql_parser.py -q
.venv\Scripts\python.exe -m relpath.cli predict "PREDICT <YENI_AGG>(transactions.amount, 0, 30, days) FOR EACH customers.customer_id" --db data/shop.duckdb
```
...
**Dosyalar:** `data/make_sample_db.py` (CLI yolu: `relpath make-sample --out data/shop.duckdb`, ki `_cmd_make_sample` `build()`'i çağırır).
...
**Amaç:** Feature'ların gelecekteki bilgiyi sızdırmadığını ispatla — relpath'in temel doğruluk garantisi.
...
**Dosyalar:** `tests/test_leakage.py`, `tests/conftest.py` (`sample_db` fixture, DB'yi geçici path'e kurar), `relpath/features.py` (`synthesize` + `cutoff_time`), `relpath/pql/compile.py` (`build_split` pencere SQL'i).
...
**Amaç:** Relational feature'ların değerini ölç — relpath'in tam relational feature'ları ile relational-olmayan baseline'ı karşılaştır.
...
.venv\Scripts\python.exe -m relpath.eval --db data\shop.duckdb --pql "PREDICT COUNT(transactions.*, 0, 30, days) == 0 FOR EACH customers.customer_id"
```

`eval.evaluate_local(db, pql)` relpath'in full relational feature'larını entity-own-columns baseline'a karşı koşar.

**Beklenen (doğrulanmış metrikler, sample DB):**

| Görev | relpath | Baseline | Delta |
|---|---|---|---|
...
**Doğrulama:** Local eval, relpath vs baseline metriklerini ve pozitif bir delta'yı yazdırmalı (churn için ~+0.045 civarı).
...
$env:RELPATH_LLM_MODEL  = "claude-sonnet-4-6"   # opsiyonel override (varsayılan zaten bu)
$env:PYTHONUTF8 = "1"
.venv\Scripts\python.exe -m relpath.cli ask "onumuzdeki ay churn edecek musteriler" --db data\shop.duckdb
```
...
**Dosyalar:** `relpath/nlp.py` (`nl_to_pql`, `NLResult`, `schema_summary`), `relpath/engine.py` (`ask()`), `relpath/cli.py` (`ask` subcommand).
...
```powershell
$env:PYTHONUTF8 = "1"
.venv\Scripts\streamlit.exe run relpath\demo_app.py
# veya:
...
**Dosyalar:** `relpath/demo_app.py`, veri için `data/shop.duckdb`.
...
**Amaç:** `docs/RELPATH_SPEC_v2.md`'yi DOCX ve HTML olarak üret; logoyu PNG'ye dönüştür.

**Bağlam (mevcut dosyalar):** `docs/RELPATH_SPEC_v2.md` + `.docx` + `.html`, ve `docs/logo.svg` + `docs/logo.png` zaten present.

**Adımlar:**

```powershell
# Markdown -> DOCX ve HTML (pandoc):
pandoc docs\RELPATH_SPEC_v2.md -o docs\RELPATH_SPEC_v2.docx
pandoc docs\RELPATH_SPEC_v2.md -s -o docs\RELPATH_SPEC_v2.html
...
**Dosyalar:** `docs/RELPATH_SPEC_v2.md` (kaynak), `docs/RELPATH_SPEC_v2.docx`, `docs/RELPATH_SPEC_v2.html`, `docs/logo.svg`, `docs/logo.png`.
...
**Dosyalar:** `relpath/schema.py` (`build_entityset`, `_normalize_dtypes` — DuckDB çıktısını `datetime64[ns]`/`object`'e coerce eder ki woodwork kabul etsin), `pyproject.toml` (pin `pandas==2.2.3`).

**Doğrulama:**

```powershell
.venv\Scripts\python.exe -m pytest tests\ -q   # 12/12 geçmeli
```
...
| Skill / Komut | Ne zaman kullan (relpath bağlamı) |
|---|---|
...
| **`/run`** | Uygulamayı başlatıp bir değişikliği canlı görmek için — CLI (`relpath predict ...`) veya Streamlit demo (A8). Testten değil gerçek app'ten teyit. |
