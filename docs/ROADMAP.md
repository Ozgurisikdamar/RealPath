...
25/25 test, sızıntı testi      RDL/GNN backend (relbench+PyG) Multimodal (metin sütun)
...
- [x] **Testler** — `tests/`: 25/25 geçiyor; `test_leakage.py` anchor sonrası satırları silip feature matrix'in aynı kaldığını ispatlıyor.
