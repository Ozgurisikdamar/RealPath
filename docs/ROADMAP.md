...
23/23 test, sızıntı testi      RDL/GNN backend (relbench+PyG) Multimodal (metin sütun)
...
- [x] **Testler** — `tests/`: 23/23 geçiyor; `test_leakage.py` anchor sonrası satırları silip feature matrix'in aynı kaldığını ispatlıyor.
