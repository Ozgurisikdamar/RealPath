...
```powershell
.venv\Scripts\python.exe -m pytest tests\ -q          # 23/23 pass
...
**Doğrulanmış metrikler** (sample DB): churn ROC-AUC ~0.749 / accuracy ~0.737 (entity-only baseline ROC-AUC ~0.704, Δ +0.045) · customer-level return-risk (2-hop join) ~0.689 ROC-AUC · product demand forecast (varsayılan 3 ay) MAE ~8.4 (bu sentetik veride ilişkisel lift yok; churn asıl gösterge). Testler: **23/23 pass**.
