...
The data has *learnable temporal structure* on purpose: a customer's recent activity (spend,
frequency, returns) genuinely predicts whether they churn — so a feature-aware model should beat
a no-feature baseline, which is the point we want to demonstrate.
"""
...
def _make_transactions(rng, customers, products):
    ...
    return tx.sort_values("tx_time").reset_index(drop=True)
...
def _make_returns(rng, transactions, cust_propensity):
    ...
    which makes future return-risk predictable from history."""
...
def build(output_path: str | Path = "data/shop.duckdb") -> Path:
    """Build the synthetic e-commerce DuckDB at ``output_path`` (deterministic, seed 42)."""
    import duckdb
